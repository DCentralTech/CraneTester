...
class BitcraneThread(QThread):
    ...
    def initialize_bitcrane(self):
        try:
            # Extract the base URL without the interface number
            base_url = self.device_url.rsplit('/', 1)[0]
            
            self.gpio = GpioSyncController()
            gpio_url = f'{base_url}/2'
            self.log_signal.emit(f"Configuring GPIO with URL: {gpio_url}")
            self.gpio.configure(gpio_url, direction=0xff, initial=0x00)
            self.gpio.exchange(0x00)
            self.gpio.exchange(0xff)
            
            serial_url = f'{base_url}/1'
            self.log_signal.emit(f"Opening serial port with URL: {serial_url}")
            self.port = pyftdi.serialext.serial_for_url(serial_url, baudrate=115200, timeout=1)
            self.log_signal.emit(str(self.port))
        except Exception as e:
...
class BitcraneTester(QMainWindow):
    def __init__(self):
        super().__init__()
        if not self.check_ftdi_drivers():
            QMessageBox.critical(self, "Error", "FTDI drivers not found. Please install the appropriate drivers.")
            sys.exit(1)
        self.initUI()
    ...
    def update_device_list(self):
        ...
        try:
            ...
            for device in devices:
                url = f"ftdi://ftdi:4232:{device[0].bus:03x}:{device[0].address:03x}"
                self.device_combo.addItem(url)
            if self.device_combo.count() == 0:
                self.device_combo.addItem("No FTDI devices found")
        except Exception as e:
            self.log_text.append(f"Error listing devices: {str(e)}")
    ...
    def start_test(self):
        ...
        if device_url == "No FTDI devices found" or device_url == "Error listing devices":
