...
class BitcraneTester(QMainWindow):
    ...
    def update_device_list(self):
        ...
        for usb in wmi.InstancesOf("Win32_PnPEntity"):
            if usb.Name is not None and "FTDI" in usb.Name:
                com_port = usb.Name.split("(")[-1].split(")")[0]
                self.device_combo.addItem(f"{usb.Name} - {com_port}")
                found_devices = True
    ...
    def start_test(self):
        ...
        com_port = device_info.split(" - ")[-1]
        device_url = f"ftdi://{com_port}"
