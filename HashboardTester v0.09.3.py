...
class BitcraneThread(QThread):
    ...
    def initialize_bitcrane(self):
        try:
            # Iterate through possible interfaces 1 to 4
            for interface in range(1, 5):
                try:
                    self.port = serial_for_url(f"{self.device_url}/{interface}", baudrate=self.baud_rate, timeout=self.timeout)
                    self.log_signal.emit(f"Connected to {self.device_url}/{interface} at {self.baud_rate} baud")
                    return
                except Exception as e:
                    self.log_signal.emit(f"Initialization error on interface {interface}: {str(e)}")
            raise Exception("Unable to initialize any interface")
        except Exception as e:
...
class BitcraneTester(QMainWindow):
    ...
    def start_test(self):
        ...
        device_url = f"ftdi://ftdi:6011/{com_port[3:]}"  # Updated device URL format
